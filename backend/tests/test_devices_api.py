...
def test_bulk_upload_processes_each_device_independently(authed_client):
    ...
    assert len(vulnerable_result["findings"]) == len(CIS_CONTROLS)

    assert "error" in broken_result
    assert "device_id" not in broken_result

    assert "device_id" in hardened_result
    assert all(f["status"] == "pass" for f in hardened_result["findings"])
...
def test_fleet_summary_is_empty_with_no_devices(authed_client):
    ...
    assert body["total_pass_count"] == 0
    assert body["total_fail_count"] == 0
    assert body["devices"] == []
    assert body["most_common_failures"] == []
...
def test_fleet_summary_aggregates_across_all_stored_devices(authed_client):
    ...
    assert body["total_pass_count"] + body["total_fail_count"] == 3 * len(CIS_CONTROLS)
    ...
    top_failure = body["most_common_failures"][0]
    assert top_failure["fail_count"] >= 2
    assert all(
        f["fail_count"] <= top_failure["fail_count"] for f in body["most_common_failures"]
    )
    failing_control_ids = {f["control_id"] for f in body["most_common_failures"]}
